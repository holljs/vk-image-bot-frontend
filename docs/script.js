// script.js (ФИНАЛЬНАЯ ЧИСТАЯ ВЕРСИЯ БЕЗ ДУБЛИКАТОВ И СЛЕШЕЙ)

const BRAIN_API_URL = 'https://neuro-master.online/api';
let USER_ID = null;
let userIdInitialized = false; // Важный флаг для таймаута
const filesByMode = {};
...
const downloadButton = document.getElementById('downloadButton'); // Наша кнопка-ссылка <a>
...
// --- Обработчики модального окна для картинок ---
if (resultImage) {
    resultImage.addEventListener('click', function() {
        if (resultImage.src && !vkBridge.isWebView()) { 
            // На ПК открываем свою модалку
            modal.style.display = "block"; 
            modalImg.src = resultImage.src; 
        } else if (resultImage.src && vkBridge.isWebView()) {
            // В мобильном ВК открываем нативную галерею
            vkBridge.send("VKWebAppShowImages", { images: [resultImage.src] });
        }
    });
}
...
// Скрываем кнопки оплаты сразу, если мобилка
...
// Если событие не пришло - берем ID силой через 2 секунды
setTimeout(() => {
    if (!userIdInitialized) {
        console.warn("Таймаут события. Запускаю запасной план.");
        vkBridge.send('VKWebAppGetUserInfo')
            .then(userInfo => {
                if (userInfo.id && !userIdInitialized) {
                    USER_ID = userInfo.id;
                    userIdInitialized = true;
                    console.log("VK User ID получен (fallback):", USER_ID);
                    updateBalance();
                }
            })
            .catch(err => console.error("Запасной план получения ID не сработал:", err));
    }
}, 2000);
...
function showCustomAlert(message, title = "Уведомление") {
    ...
    if (customModal && messageEl) { 
        if (titleEl) titleEl.textContent = title; 
        messageEl.textContent = message; 
        customModal.classList.remove('hidden'); 
        document.body.classList.add('modal-open'); 
    } else {
        alert(title + "\n" + message); // Запасной вариант
    }
}
...
document.querySelectorAll('.process-button').forEach(btn => {
    btn.addEventListener('click', async (event) => { 
        ...
        if (mode === 'music') { 
            musicLyrics = prompt; 
            if (event.target.dataset.style) { // Заменили btn на event.target
        ...
        if (mode === 'music') { 
            ...
            if (event.target.dataset.style === 'custom') { 
