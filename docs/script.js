// script.js (v118 - ИСПРАВЛЕНА ИНИЦИАЛИЗАЦИЯ И ПОДПИСЬ)
...
function updateBalance() {
    ...
    // ИСПРАВЛЕНИЕ: ДОБАВЛЕН ЗАГОЛОВОК X-VK-Sign ДЛЯ ИНИЦИАЛИЗАЦИИ
...
function showCustomAlert(message, title = "Уведомление") {
    ...
    if (modal && messageEl) {
        ...
        alert(title + ": " + message); // Фолбэк, если модалки нет
    }
}
        ...
        try {
            ...
            const response = await fetch(endpoint, {
                ...
                headers: { 
                    ...
                    'X-VK-Sign': getAuthHeader() // ИСПРАВЛЕНИЕ: ДОБАВЛЕНА ПОДПИСЬ
                },
            ...
            if (response.ok) {
                if (mode === 'chat') {
                    ...
                    // Прямой ответ сервера (без очереди)
                    hideLoader();
                    showResult(result);
                    updateBalance();
                }
